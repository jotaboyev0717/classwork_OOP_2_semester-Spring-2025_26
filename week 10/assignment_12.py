...
class BlendPlanner:
    ...
    def scale_blend(self, name, desired_plots):
        ...
        minerals = blend["minerals"]
        ratio = desired_plots / base_plots

        scaled = {}
        for mineral, amount in minerals.items():
            scaled[mineral] = round(amount * ratio, 2)
    ...
    def check_stock(self, name, stock):
        ...
        for mineral, needed_amount in minerals.items():
            available_amount = stock.get(mineral, 0)

            if available_amount < needed_amount:
                short_amount = round(needed_amount - available_amount, 2)
                missing[mineral] = short_amount
